//TAILWIND CONFIG - Intégration avec variables CSS du site
...
// Configuration couleurs synchronisées avec CSS
const colors = {
    ...
    primaryLight: () => getComputedVar('--primary-light'),
    muted: () => getComputedVar('--muted'),
    mutedForeground: () => getComputedVar('--muted-foreground'),
    accentRed: () => getComputedVar('--accent-red'),
    accentCyan: () => getComputedVar('--accent-cyan'),
    accentYellow: () => getComputedVar('--accent-yellow'),
    border: () => getComputedVar('--border'),
};
...
const Game = {
    ...
    // State
    maze: [],
    player: { x: 1, y: 1, nextDir: null, currentDir: null, moveProgress: 0, speed: 7.0, lastStepTime: 0 },
    enemy: { x: 19, y: 13, moveTimer: 0, moveInterval: 300, lastPos: { x: -1, y: -1 } },
    ...
    update: function (dt, timestamp) { // Accepte le timestamp
        ...
        if (pct < 20) bar.style.backgroundColor = 'var(--accent-red)';
    ...
    draw: function () {
        ...
        ctx.shadowBlur = 10;
        ctx.shadowColor = colors.accentYellow();
...
function initLightMode() {
    ...
    setInterval(() => {
        ...
        const hasLightClass = document.body.classList.contains('light-mode');

...
const game = {
    ...
    nextTurn: function () {
        ...
        const diffMult = 1 + ((this.hpBossMax - this.hpBoss) / (this.hpBossMax * 2.5));

        let config = {};
        switch(type) {
            case 'mash':
                config = { type: 'mash', time: 4000, label: "FRAPPEZ !", target: Math.ceil(2 * diffMult) };
                break;
            case 'sequence':
                config = { type: 'sequence', time: 5000, label: "CONTRE-SORT", length: 2 + Math.floor(diffMult - 1) };
                break;
            case 'timing':
                config = { type: 'timing', time: 3000, label: "PRÉCISION", speed: 1.8 * diffMult };
                break;
            case 'reflex':
                config = { type: 'reflex', time: 4000, label: "BLOQUEZ !", count: 2 };
                break;
            case 'hold':
    ...
    startQTE: function(config) {
        ...
        setTimeout(() => { this.inputLocked = false; }, 800);
        this.startTimer(config.time + 800);
    ...
    success: function () {
        if (!this.isRunning || this.qteResolved) return;
    ...
    fail: function (reason) {
        ...
        if (this.hpPlayer <= 0) {
            ...
            document.getElementById('fail-reason').innerText = "U Strigone vous a vaincu.";
    ...
    exit: function (win) {
        ...
        if (window.finishGame) {
            ...
            ((new URLSearchParams(window.location.search)).get('from')==='minigames' ? window.location.href='../../../index.html#univers-prosa/mini-jeux' : history.back());
        }
    },

    updateHealthUI: function() {
        document.getElementById('boss-hp').style.width = `${(this.hpBoss / this.hpBossMax) * 100}%`;
...
const qteTiming = {
    ...
    stop: function () {
        if (!game.isRunning || game.inputLocked) return;
...
const qteHold = {
    ...
    startHold: function() { if (game.inputLocked) return; this.holding = true; this.btn.classList.add('holding'); },

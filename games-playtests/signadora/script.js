...
class Game {
    ...
    update() {
        ...
        if (this.inputActive) {
            ...
            this.player.speed = Math.max(this.player.speed - 0.001, 0);
        }
    ...
    switchGuardState() {
        ...
        switch (this.guard.state) {
            case GUARD_STATE.SAFE:
                ...
                this.guard.nextSwitch = 25 + Math.random() * 30; 
                break;

            case GUARD_STATE.WARN:
                this.guard.state = GUARD_STATE.DANGER;
                this.guard.nextSwitch = 90 + Math.random() * 90; 
                alertText.style.opacity = 1;
                break;

            case GUARD_STATE.DANGER:
                this.guard.state = GUARD_STATE.SAFE;
                this.guard.nextSwitch = 40 + Math.random() * 100; 
